...
public class UpdateLocationCommentHandler : ICommandHandler<UpdateLocationComment>
{
    private readonly IMongoRepository<LocationCommentDocument, Guid> _repository;

    public UpdateLocationCommentHandler(IMongoRepository<LocationCommentDocument, Guid> repository)
    {
        _repository = repository;
    }

    public async Task HandleAsync(UpdateLocationComment command, CancellationToken cancellationToken = default)
    {
        if (command is null)
        {
