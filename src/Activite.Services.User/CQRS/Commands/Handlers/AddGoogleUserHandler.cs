...
public class AddGoogleUserHandler : ICommandHandler<AddGoogleUser>
{
    private readonly IMongoRepository<GoogleUserDocument, Guid> _repository;

    public AddGoogleUserHandler(IMongoRepository<GoogleUserDocument, Guid> repository)
    {
        _repository = repository;
    }

    public async Task HandleAsync(AddGoogleUser command, CancellationToken cancellationToken = default)
    {
        if (command is null)
        {
        ...
        var user = new GoogleUserDocument
        {
            Id = command.Id,
            Email = command.Email,
